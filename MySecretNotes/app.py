...
def notes():
    ...
    if request.method == 'POST':
        if request.form['submit_button'] == 'add note':
            ...
            db = connect_db()
            c = db.cursor()
            stmt = "INSERT INTO notes(id, assocUser, dateWritten, note, publicID) VALUES (NULL, ?, ?, ?, ?);"
            params = (session['userid'], time.strftime('%Y-%m-%d %H:%M:%S'), note, random.randrange(1000000000, 9999999999))
            print(stmt)
            ...
            db = connect_db()
            c = db.cursor()
            stmt = "SELECT * FROM notes WHERE publicID = ?"
            c.execute(stmt, (noteid,))
            result = c.fetchall()
            if(len(result)>0):
                row = result[0]
                stmt = "INSERT INTO notes(id, assocUser, dateWritten, note, publicID) VALUES (NULL, ?, ?, ?, ?);"
                params = (session['userid'], row[2], row[3], row[4])
                c.execute(stmt, params)
            else:
    ...
    db = connect_db()
    c = db.cursor()
    stmt = "SELECT * FROM notes WHERE assocUser = ?;"
    print(stmt)
    c.execute(stmt, (session['userid'],))
    notes = c.fetchall()
    print(notes)
...
def register():
    ...
    if request.method == 'POST':
        ...
        db = connect_db()
        c = db.cursor()
        user_stmt = """SELECT * FROM users WHERE username = ?;""" 
        c.execute(user_stmt, (username,))
        if(len(c.fetchall())>0):
            errored = True
            usererror = "That username is already in use by someone else!"

        if(not errored):
            stmt = """INSERT INTO users(id,username,password) VALUES(null, ?, ?);"""
            print(stmt)
            c.execute(stmt, (username, hashed))
            db.commit()
            db.close()
            return f"""<html>
                        <head>
